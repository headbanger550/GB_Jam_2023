...
public class UpgradeSystem : MonoBehaviour
{
    ...
    [SerializeField] int scoreRemoveAmount;
    ...
    void GetWeapon(GameObject arm)
    {
        player = arm.GetComponentInParent<Player>();
    ...
    void DisplayUpgrade(int upgrdType)
    {
        switch(upgrdType)
        {
            ...
            case 1:
                upgradeImage.sprite = weaponObj.upgradeImages[1];
                descriptionText.text = weaponObj.descriptions[1];
                break;
        }
    }
    ...
    {
        if(player.scoreAmmount <= scoreRemoveAmount)
        {
            player.scoreAmmount -= scoreRemoveAmount;
